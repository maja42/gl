...
func CheckFramebufferStatus(target Enum) Enum {
	var status Enum
	ctx.enqueue(true, func() {
		status = Enum(gl.CheckFramebufferStatus(uint32(target)))
	})
	return status
}
...
func CreateProgram() Program {
	var p Program
	ctx.enqueue(true, func() {
		p = Program{Value: gl.CreateProgram()}
	})
	return p
}
...
func CreateShader(ty Enum) Shader {
	var s Shader
	ctx.enqueue(true, func() {
		s = Shader{Value: gl.CreateShader(uint32(ty))}
	})
	return s
}
...
func GetActiveAttrib(p Program, index uint32) (name string, size int, ty Enum) {
	...
	cname := gl.Str(name)
	ctx.enqueue(true, func() {
		gl.GetActiveAttrib(p.Value, index, int32(len(name)-1), &length, &si, &typ, cname)
	})
...
func GetAttachedShaders(p Program) []Shader {
	...
	var buf []uint32
	ctx.enqueue(true, func() {
		var shadersLen int32
		gl.GetProgramiv(p.Value, uint32(ATTACHED_SHADERS), &shadersLen)

		var n int32
		buf = make([]uint32, shadersLen)
		gl.GetAttachedShaders(p.Value, shadersLen, &n, &buf[0])
		buf = buf[:int(n)]
	})

	shaders := make([]Shader, len(buf))
	for i, s := range buf {
		shaders[i] = Shader{Value: s}
	}
	return shaders
}
...
func GetAttribLocation(p Program, name string) Attrib {
	var a Attrib
	ctx.enqueue(true, func() {
		a = Attrib{Value: uint(gl.GetAttribLocation(p.Value, gl.Str(name+"\x00")))}
	})
	return a
}
...
func GetShaderSource(s Shader) string {
	...
	ctx.enqueue(true, func() {
		var sourceLen int32
		gl.GetShaderiv(s.Value, gl.SHADER_SOURCE_LENGTH, &sourceLen)
		if sourceLen == 0 {
			return
		}
...
func GetString(pname Enum) string {
	var s string
	ctx.enqueue(true, func() {
		s = gl.GoStr(gl.GetString(uint32(pname)))
	})
	return s
}
...
func GetUniformLocation(p Program, name string) Uniform {
	var u Uniform
	ctx.enqueue(true, func() {
		u = Uniform{Value: gl.GetUniformLocation(p.Value, gl.Str(name+"\x00"))}
	})
	return u
}
